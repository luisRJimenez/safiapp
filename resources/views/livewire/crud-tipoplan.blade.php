<div>
    ...
    <div class="max-w-7xl mx-auto py-2 px-4 sm:px-6 lg:px-8 flex">
        ...
        <div class="pr-2">
            ...
            <x-jet-input type="text" placeholder="Tipo de plan" wire:model.defer="tpldescripcion" required></x-jet-input>
            <x-jet-input-error for="tpldescripcion"/>
        </div>

        <div class="pr-2">
           <x-jet-input type="number" placeholder="puntos contado" wire:model.defer="tplpuntoscontado" required></x-jet-input>
            <x-jet-input-error for="tplpuntoscontado"/>
        </div>

        <div class="pr-2">
            <x-jet-input type="number" placeholder="puntos credito" wire:model.defer="tplpuntoscredito" required></x-jet-input>
             <x-jet-input-error for="tplpuntoscredito"/>
        </div>
        ...
        <div class="py-1">
            ...
            @else
                <x-jet-button disabled wire:dirty.attr.remove="disabled" wire:target="tpldescripcion" wire:click='salvar()'>Grabar</x-jet-button>
            @endif
        </div>
    </div>
                ...
                @forelse ($data as $item)
                  <tr>
                    <td class="px-6 py-1 whitespace-nowrap">
                        <div class="ftext-sm text-gray-900">{{$item ->id}}</div>
                    </td>
                    <td class="px-6 py-1 whitespace-nowrap">
                      <div class="text-sm text-gray-900">{{$item ->tpldescripcion}}</div>
                    </td>
                    <td class="px-6 py-1 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{$item ->tplpuntoscontado}}</div>
                    </td>
                    <td class="px-6 py-1 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{$item ->tplpuntoscredito}}</div>
                    </td>
                  ...
                  @empty
                    <tr class="text-center">
                        <td colspan="6" class="py-3 italic">No hay información</td>
                    </tr>
                  @endforelse
                  <!-- More items... -->
                </tbody>
