    ...
    <div class="max-w-7xl mx-auto py-2 px-4 sm:px-6 lg:px-8 grid grid-rows-1 grid-flow-col gap-4">
            <div class="flex space-x-2">
                <div>
                    ...
                    <select wire:model.defer='selectedtipomov' class=" text-gray-500 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                        <option value="">-- Tipo Movimiento --</option>
                        @foreach ($tipomov as $tipo)
                            <option value="{{$tipo}}">{{$tipo == 1 ? 'INGRESO' : 'EGRESO'}}</option>
                        @endforeach
                    </select>
                    <x-jet-input-error for="selectedtipomov" wire:dirty.attr="hidden" wire:target="selectedtipomov"/>
                </div>

                <div>
                    <x-jet-input type="text" placeholder="Concepto Movimiento" wire:model.defer="cpmdescripcion" required></x-jet-input>
                    <x-jet-input-error for="cpmdescripcion" wire:dirty.attr="hidden" wire:target="cpmdescripcion"/>
                </div>
                ...
                <div class="py-1">
                    ...
                    @else
                        <x-jet-button disabled wire:dirty.attr.remove="disabled" wire:target="cpmdescripcion" wire:click='salvar()'>Grabar</x-jet-button>
                    @endif
                </div>
            </div>
        ...
        <div class="justify-self-end place-self-start">
            <div class=" relative rounded-md shadow-sm">
                <div class="absolute inset-y-0 left-0 pl-1 flex items-center pointer-evencpm-none">
                ...
                @forelse ($data as $item)
                  <tr>
                    ...
                    <td class="px-6 whitespace-nowrap">
                      <div class="text-sm text-gray-900">{{$item ->cpmdescripcion}}</div>
                    </td>
                    <td class="px-6 whitespace-nowrap">
                        <div class="text-sm text-gray-900">
                          {{ $item ->cpmtipomovimiento == 1 ? 'INGRESO' : 'EGRESO'}}
